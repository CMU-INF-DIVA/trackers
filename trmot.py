...
class TRMOTTracker(Tracker):
    ...
    def __init__(self, type_names, video_name, frame_rate, max_age=2,
                 min_iou=0.2, feature_thres=0.7, feature_buffer_size=1):
        super(TRMOTTracker, self).__init__(type_names, video_name, frame_rate)
        STrack.reset_id()
        self.trackers = {}
    ...
    def group_detection(self, detection):
        grouped_detection = defaultdict(list)
        for obj_i in range(len(detection)):
            obj_type = self.type_names(detection.object_types[obj_i].item())
            bbox = detection.object_types.tensor[obj_i].numpy()
            ...
            grouped_detection[obj_type].append(detection)
    ...
    def get_tracked_detection(self, detection):
        ...
        return detection, finished_track_ids

    def __call__(self, detection):
        grouped_detection = self.group_detection(detection)
        for obj_type, tracker in self.trackers.items():
            tracker.update(grouped_detection[obj_type])
        detection, finished_track_ids = self.get_tracked_detection(detection)
        return detection, finished_track_ids
